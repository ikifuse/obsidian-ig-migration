def generate_4layer_yaml(p_info):
    content_val = '"video"' if p_info.get("content_type") == "video" else "null"
...
group_id: null"""
    ...
    geo_lat = p_info.get("geo_lat")
    geo_lng = p_info.get("geo_lng")
    geo_lat_value = geo_lat if geo_lat is not None else "null"
    geo_lng_value = geo_lng if geo_lng is not None else "null"
    geo_confidence = "high" if (p_info['loc_name_raw'] or (geo_lat is not None and geo_lng is not None)) else "null"
