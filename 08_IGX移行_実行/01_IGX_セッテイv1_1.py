...
def resolve_output_dir(default_name):
    ...
    if os.path.basename(os.path.normpath(resolved)) == "output_IGC":
        raise ValueError("IGXからoutput_IGCへ直接出力することは禁止されています。")
    return resolved
...
# 独立した出力先（サルベージ検証用）
DEST_VAULT_DIR = resolve_output_dir("output_IGX")
...
DEST_SALVAGE_DIR = os.path.join(LOGS_ROOT, "Salvage")
DEST_MEDIA_DIR = os.path.join(LOGS_ROOT, "media")
EVENTS_DIR = os.path.join(LOGS_ROOT, "SystemLogs", "Events")

STATE_FILE = os.path.join(LOGS_ROOT, "migration_state_11_salvage.json")
