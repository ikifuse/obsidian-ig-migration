...
test("融合済み絞り込みだけは構成カードを個別に表示する", () => {
  const result = 表示一覧を作る(state(), { kind: "all", status: "merged", handwrittenOnly: false, search: "" });
  assert.equal(result.total, 2);
  assert.deepEqual(result.items.map((item) => item.cardIds[0]), ["m1", "l1"]);
});

test("融合グループの検索は全構成カードを対象にする", () => {
  const result = 表示一覧を作る(state(), { kind: "all", status: "all", handwrittenOnly: false, search: "場所" });
