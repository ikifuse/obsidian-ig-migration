...
event_at_ms: null"""
    ...
    tags_yaml = "\n".join([f"  - \"{t}\"" for t in p_info['tags']]) if p_info['tags'] else " []"
...
group_id: null"""
    ...
    loc_normalized = f'"{p_info["loc_name_raw"]}"' if p_info['loc_name_raw'] else "null"
    ...
    synapse_link_val = f'"[[{p_info["loc_name_safe"]}]]"' if p_info['loc_name_safe'] else "null"
  ...
  synapse_link: {synapse_link_val}"""
    ...
    if media_items:
        media_yaml_items = "\n".join(
            [f"  - path: \"{m['path']}\"" for m in media_items]
        )
